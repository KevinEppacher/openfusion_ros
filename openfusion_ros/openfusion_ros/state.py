...
try:
    ...
    import time
    from openfusion_ros.utils.utils import rand_cmap
    DBG = False
except:
...
class VLState(BaseState):
    ...
    @torch.no_grad()
    def fast_object_query(self, t_emb, points, colors=None, only_poi=False, topk=1, return_scores=False):
        buf_indices = self.world.hashmap().active_buf_indices()
        buf_indices = torch.utils.dlpack.from_dlpack(buf_indices.to_dlpack())

        mask_key = self.emb_keys[buf_indices.cpu()].view(-1, self.num_obj_points_per_block)
        ...
        mask_cls = torch.einsum("cd,nd->cn", t_emb, mask_pred_caption)  # [1, num_keys]
        value, indices = torch.topk(mask_cls, topk, dim=1)  # scores + index of top-k keys

        selected_keys = valid_keys[indices.flatten(0).cpu()]  # keys that are active
        selected_scores = value.flatten().cpu().numpy()       # their similarity scores
        ...
        # Get all POI points
        all_poi = self.emb_coords[buf_indices.cpu()].view(
            -1, self.num_obj_points_per_block, 3
        )
        poi = all_poi[key_loc.unsqueeze(-1).repeat(1,1,3)].view(-1,3).numpy()

        if return_scores:
            # Now map each key to its score
            key_to_score = {k.item(): s for k, s in zip(selected_keys, selected_scores)}
            key_labels = mask_key[key_loc]  # which key each POI point came from
            point_scores = np.array([key_to_score[k.item()] for k in key_labels.view(-1)])

            if only_poi:
                return poi, point_scores

            points = np.concatenate([points, poi], axis=0)
            colors = np.concatenate([colors, np.ones_like(poi) * np.array([[255, 0, 0]])], axis=0)
            return points, colors, point_scores
    ...
    def object_query(
        ...
        """
        Perform a dense semantic query over a 3D point cloud.

        Args:
            t_emb (torch.Tensor): Text query embedding.
            points (np.ndarray): 3D points [N, 3].
            colors (np.ndarray): RGB values for each point [N, 3].
            only_poi (bool): If True, return only points of interest.
            topk (int): Top-k semantic matches to consider.
            dist_thresh (float): Distance threshold for neighborhood matching.
            count_thresh (int): Minimum number of neighbors for a point to be considered relevant.

        Returns:
            If only_poi:
                - poi_points (np.ndarray): Points of interest.
                - poi_scores (np.ndarray): Associated similarity scores.
            Else:
                - all_points (np.ndarray): Full set of input points.
                - all_colors (np.ndarray): Colors, with relevant points painted red.
                - mask (np.ndarray): Boolean mask indicating relevant points.
                - poi_scores (np.ndarray): Similarity scores for POIs.
        """
        # Get semantically relevant points and their scores
        poi, scores = self.fast_object_query(
            t_emb,
            points,
            colors,
            only_poi=True,
        ...
        # Build a KD-Tree over the POIs for fast spatial lookup
        tree = KDTree(poi, leaf_size=10)
        dist, ind = tree.query(points, k=10)

        # Mask: a point is relevant if it has enough close POIs
        mask = np.sum(dist <= dist_thresh, axis=1) > count_thresh

        if only_poi:
            return points[mask], scores  # Only return POIs and scores if requested

        # Optionally color the relevant points in red
        if colors is not None:
            colors[mask] = np.array([255, 0, 0])

        return points, colors, mask, scores
